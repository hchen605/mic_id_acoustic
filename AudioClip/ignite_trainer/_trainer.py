...
def run(experiment_name: str,
    ...
    with _utils.tqdm_stdout(orig_stdout) as orig_stdout:
        ...
        def training_step(engine: ieng.Engine, batch) -> torch.Tensor:
            ...
            #print(len(audio))
            #print(text)
            text = [ [text[0][i]] for i, _ in enumerate(text[0]) ]
            #print(text)
            ...
            with torch.no_grad():
                ...
                ((_, _, text_features), _), _ = model(
                    text=[
                        ...
                        #[eval_loader.dataset.label_set[i]] for i, _ in enumerate(eval_loader.dataset.label_set)
                    ],
                    batch_indices=torch.arange(
                        len(eval_loader.dataset.class_idx_to_label), dtype=torch.int64, device=device
                        #len(eval_loader.dataset.label_set), dtype=torch.int64, device=device
                    )
                )
